    ...
    async updateTradingTimes() {
        ...
        if (!api_base.api) {
            await api_base.init().catch(error => {
                console.error('[Bot Builder] Failed to initialize websocket for trading times.', error);
            });
        }
        ...
        if (api_base.api === ws && typeof api_base.waitForSocketOpen === 'function') {
            try {
                ...
                console.error('[Bot Builder] Trading times websocket did not open in time.', error);
                return;
            }
        }

        const response = await ws.send({ trading_times: last_update_date }).catch(error => {
            const log = error?.error?.code === 'RateLimit' ? console.warn : console.error;
            log('[Bot Builder] Failed to fetch trading times.', error);
            return null;
        });

        if (!response || response.error) {
            if (response?.error?.code === 'RateLimit') {
                console.warn('[Bot Builder] Trading times request was rate-limited; keeping current market status.');
            }
            return;
        }
