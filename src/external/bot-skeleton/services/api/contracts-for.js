...
const getDropdownValue = options => (Array.isArray(options?.[0]) ? options[0][1] : 'na');

const hasDropdownValue = (options, value) =>
    Array.isArray(options) && options.some(option => Array.isArray(option) && option[1] === value);

const PROFITDOCK_CONTRACTS_CACHE_PREFIX = 'profitdock.contracts_for.';
const PROFITDOCK_CONTRACTS_CACHE_VERSION = 'v2';
...
export default class ContractsFor {
    ...
    getCacheKey(symbol) {
        return `${PROFITDOCK_CONTRACTS_CACHE_PREFIX}${PROFITDOCK_CONTRACTS_CACHE_VERSION}.${symbol}`;
    }

    readCachedContractsFor(symbol) {
        if (!isProfitdockDomain() || typeof window === 'undefined') {
            return null;
        }

        try {
            const cached = JSON.parse(window.localStorage.getItem(this.getCacheKey(symbol)) || 'null');
            if (!cached || !Array.isArray(cached.contracts) || !cached.contracts.length || !cached.timestamp) {
                return null;
            }

            return cached;
        } catch (error) {
            console.warn('[Bot Builder] Could not read cached contract metadata.', error);
            return null;
        }
    }

    writeCachedContractsFor(symbol, contracts) {
        if (!isProfitdockDomain() || typeof window === 'undefined' || !Array.isArray(contracts) || !contracts.length) {
            return;
        }

        try {
            window.localStorage.setItem(
                this.getCacheKey(symbol),
                JSON.stringify({
                    contracts,
            ...
            console.warn('[Bot Builder] Could not cache contract metadata.', error);
        }
    }

    rememberContractsFor(symbol, contracts, timestamp = this.server_time.unix()) {
        this.contracts_for[symbol] = {
            contracts,
            timestamp,
        };
        this.writeCachedContractsFor(symbol, contracts);
    }

    getUsableCachedContracts(symbol) {
        const memory_cache = this.contracts_for[symbol];

        if (memory_cache?.contracts?.length) {
            return memory_cache;
        }

        const persisted_cache = this.readCachedContractsFor(symbol);

        if (persisted_cache?.contracts?.length) {
            this.contracts_for[symbol] = {
                contracts: persisted_cache.contracts,
                timestamp: persisted_cache.timestamp,
            };
            return this.contracts_for[symbol];
        }

        return null;
    }

    async getContractsForFromProfitdockApi(symbol) {
        if (!isProfitdockDomain() || typeof window === 'undefined') {
            return null;
        }

        const response = await fetch(`/api/deriv/options/contracts-for?symbol=${encodeURIComponent(symbol)}`, {
            cache: 'no-store',
        ...
        if (!response.ok) {
            throw new Error(payload?.message || 'ProfitDock could not load Deriv contract metadata.');
        }

        const contracts = payload?.contracts;
        return Array.isArray(contracts) && contracts.length ? contracts : null;
    }

    async getFallbackContractsFor(symbol) {
        try {
            const contracts = await this.getContractsForFromProfitdockApi(symbol);
            ...
            const filtered_contracts = contracts
                .map(contract => this.normalizeContractForOptionsApi(contract))
                .filter(c => c.start_type !== 'forward');

            this.rememberContractsFor(symbol, filtered_contracts);
            window.setTimeout(() => this.refreshTradeDefinitionBlocks(symbol), 0);

            return filtered_contracts;
        } catch (error) {
            console.warn('[Bot Builder] ProfitDock contract metadata fallback failed.', error);
            return [];
        }
    }

    normalizeContractForOptionsApi(contract) {
        if (!contract || !isProfitdockDomain()) {
            return contract;
        }

        return {
            ...contract,
            barrier_category: contract.barrier_category || this.getBarrierCategoryByTradeType(contract.contract_category),
            contract_category_display:
                contract.contract_category_display || this.getTradeTypeCategoryNameByTradeType(contract.contract_category),
            contract_display: contract.contract_display || contract.contract_type,
            start_type: contract.start_type || 'spot',
        };
    }

    refreshTradeDefinitionBlocks(symbol) {
        if (typeof window === 'undefined' || !window.Blockly?.derivWorkspace) {
            return;
        }

        const workspace = window.Blockly.derivWorkspace;
        const event_group = `profitdock-contracts-${Date.now()}`;
        ...
        workspace.getAllBlocks?.(false)?.forEach(block => {
            ...
            const top_parent_block = block.getTopParent?.();
            ...
            if (!market_block || !contract_type_block || market_block.getFieldValue('SYMBOL_LIST') !== symbol) {
                return;
            }

            const market = market_block.getFieldValue('MARKET_LIST');
            const submarket = market_block.getFieldValue('SUBMARKET_LIST');
            const category_field = block.getField('TRADETYPECAT_LIST');
            const trade_type_field = block.getField('TRADETYPE_LIST');
            const contract_type_field = contract_type_block.getField('TYPE_LIST');

            if (!category_field || !trade_type_field || !contract_type_field) {
                return;
            }

            const current_category = category_field.getValue?.();
            const current_trade_type = trade_type_field.getValue?.();
            const current_contract_type = contract_type_field.getValue?.();

            this.getTradeTypeCategories(market, submarket, symbol)
                .then(categories => {
                    if (!Array.isArray(categories) || !categories.length) {
                        return null;
                    }

                    const category = hasDropdownValue(categories, current_category)
                        ? current_category
                        : getDropdownValue(categories);
                    ...
                    return this.getTradeTypes(market, submarket, symbol, category).then(trade_types => ({
                        category,
                        trade_types,
                    }));
                })
                .then(result => {
                    if (!result || !Array.isArray(result.trade_types) || !result.trade_types.length) {
                        return;
                    }

                    const trade_type = hasDropdownValue(result.trade_types, current_trade_type)
                        ? current_trade_type
                        : getDropdownValue(result.trade_types);

                    trade_type_field.updateOptions(result.trade_types, {
                    ...
                    const opposites = config().opposites[trade_type?.toUpperCase?.()] || [];

                    if (opposites.length > 1) {
                        contract_type_options.push([localize('Both'), 'both']);
                    }

                    contract_type_options.push(...opposites.map(type => Object.entries(type)[0].reverse()));

                    if (!contract_type_options.length) {
                        contract_type_options.push(...config().NOT_AVAILABLE_DROPDOWN_OPTIONS);
                    }

                    const contract_type = hasDropdownValue(contract_type_options, current_contract_type)
                        ? current_contract_type
                        : getDropdownValue(contract_type_options);
                    ...
                    contract_type_block.onchange?.(synthetic_event);
                    top_parent_block.getChildByType?.('trade_definition_tradeoptions')?.onchange?.(synthetic_event);
                    top_parent_block.getChildByType?.('trade_definition_accumulator')?.onchange?.(synthetic_event);
                })
                .catch(error => {
                    console.warn('[Bot Builder] Could not refresh trade dropdowns after contracts metadata loaded.', error);
                });
        });
    }

    updateStore({ ws, server_time }) {
        if (ws) {
            this.ws = ws;
        }
    ...
    async getBarriers(symbol, trade_type, duration, barrier_types) {
        ...
        if (contracts_for_category.length > 0) {
            barrier_types.forEach((barrier_type, index) => {
                ...
                const allowed_contract_types = this.getAllowedContractTypesByTradeType(trade_type);
                ...
                let contract = contracts_for_category.find(c => {
                    ...
                    const has_matching_contract_type =
                        !allowed_contract_types.length || allowed_contract_types.includes(c.contract_type);
                    const has_matching_barrier_category =
                        !barrier_category || !c.barrier_category
                            ? has_matching_contract_type
                            : c.barrier_category === barrier_category;
    ...
    getAllowedContractTypesByTradeType(trade_type) {
        const { opposites } = config();
        const trade_type_key = String(trade_type || '').toUpperCase();
        return (opposites[trade_type_key] || []).map(opposite => Object.keys(opposite)[0]);
    }
    ...
    async getContractsByTradeType(symbol, trade_type) {
        ...
        return contracts.filter(contract => {
            ...
            const has_matching_contract_type =
                !allowed_contract_types.length || allowed_contract_types.includes(contract.contract_type);
    ...
    async getContractsFor(symbol) {
        ...
        const getContractsForFromApi = async () => {
            ...
            try {
                const cached_contracts = this.getUsableCachedContracts(symbol);
                const now_ms = Date.now();

                if (this.rate_limited_until[symbol] && this.rate_limited_until[symbol] > now_ms) {
                    return cached_contracts?.contracts || [];
                }

                if (!api_base.api) {
                    await api_base.init().catch(error => {
                        console.error('[Bot Builder] Failed to initialize websocket for contracts.', error);
                    });
                }
                ...
                if (typeof api_base.waitForSocketOpen === 'function') {
                    try {
                        const wait_timeout = window.location.hostname.includes('profitdock.site') ? 15000 : 8000;
                        await api_base.waitForSocketOpen(wait_timeout);
                    } catch (error) {
                        console.error('[Bot Builder] Contracts websocket did not open in time.', error);
                        return [];
                    }
                }

                const requestContractsFor = async attempt => {
                    const response = await api_base.api.send({ contracts_for: symbol }).catch(error => {
                        const log = error?.error?.code === 'RateLimit' ? console.warn : console.error;
                        log(`[Bot Builder] Failed to fetch contracts_for on attempt ${attempt}.`, error);
                ...
                let response = await requestContractsFor(1);

                if (response?.error?.code === 'RateLimit') {
                    this.rate_limited_until[symbol] = Date.now() + RATE_LIMIT_BACKOFF_MS;
                    if (cached_contracts?.contracts?.length) {
                        return cached_contracts.contracts;
                    }

                    const fallback_contracts = await this.getFallbackContractsFor(symbol);
                    if (fallback_contracts.length) {
                        return fallback_contracts;
                    }
                } else if (!response?.error && !response && window.location.hostname.includes('profitdock.site')) {
                    await new Promise(resolve => setTimeout(resolve, 750));
                    response = await requestContractsFor(2);
                }
