...
class DBot {
    ...
    async initWorkspace(public_path, store, api_helpers_store, is_mobile, is_dark_mode) {
        ...
        let recent_files = [];
        try {
            const stored_workspaces = await getSavedWorkspaces();
            recent_files = Array.isArray(stored_workspaces) ? stored_workspaces : [];
        } catch (error) {
            console.warn('[Blockly] Failed to restore saved workspaces. Starting with the default template.', error);
        }
    ...
    generateCode(limitations = {}) {
        return `
            ...
            var BinaryBotPrivateTradeEachTickEnabled = false;
            ...
            while (true) {
                ...
                BinaryBotPrivateTickAnalysis();
                if (!BinaryBotPrivateRun(BinaryBotPrivateAfterPurchase) && !BinaryBotPrivateTradeEachTickEnabled) {
                    break;
                }
            }
