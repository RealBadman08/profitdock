...
window.Blockly.JavaScript.javascriptGenerator.forBlock.trade_definition = block => {
    ...
    const fallback_loginid = client?.loginid || stored_loginid;
    const client_token_candidate = client?.getToken?.(fallback_loginid);
    const selected_account_token =
        typeof client_token_candidate === 'string' && client_token_candidate !== 'null'
            ? client_token_candidate
            : '';
    const fallback_token = selected_account_token || stored_token;
    const normalized_token = typeof fallback_token === 'string' ? fallback_token : String(fallback_token || '');
    const has_profitdock_session = is_profitdock_domain && !!fallback_loginid && !!normalized_token;

    if (!client || (!client.is_logged_in && !has_profitdock_session)) {
        throw new Error('Please login');
    }

    const loginid = fallback_loginid;
    const account = normalized_token;
    ...
    const should_trade_each_tick = restart_on_buy_sell_block.getFieldValue('TIME_MACHINE_ENABLED') !== 'FALSE';
    ...
    BinaryBotPrivateInit = function BinaryBotPrivateInit() {
        Bot.init('${account}', {
          ...
          tradeEachTickEnabled: ${should_trade_each_tick},
          timeMachineEnabled  : ${should_trade_each_tick},
        });
        ${initialization.trim()}
    };
