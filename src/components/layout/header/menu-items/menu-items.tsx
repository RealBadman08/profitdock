...
export const MenuItems = observer(() => {
    ...
    const getModifiedHref = (originalHref?: string) => {
        ...
        try {
            redirect_url = new URL(originalHref, window.location.origin);
        } catch (error) {
            console.error('[Header] Skipping malformed menu link:', originalHref, error);
            return window.location.origin;
        }
    ...
    const filtered_items = items.filter((item, index) => {
        if (!item || typeof item.label !== 'string' || !item.label.trim() || typeof item.href !== 'string' || !item.href.trim()) {
            return false;
        }
                    ...
                    : // For mobile, show the first available item after filtering
                      filtered_items.length > 0 && (
                          <MenuItem
                              ...
                              key={`${filtered_items[0].label}-${filtered_items[0].href}`}
...
export const TradershubLink = observer(() => {
    ...
    return (
        ...
        >
            <Text>{localize(TRADERS_HUB_LINK_CONFIG.label)}</Text>
        </MenuItem>
    );
});
