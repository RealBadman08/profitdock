...
const TradeAnimation = observer(({ className, should_show_overlay }: TTradeAnimation) => {
    ...
    const [, setProfitdockStatusVersion] = React.useState(0);
    ...
    React.useEffect(
        () =>
            subscribeProfitdockTradeStatus(() => {
                setProfitdockStatusVersion(version => version + 1);
            }),
        []
    );
    ...
    const active_profitdock_feature =
        safeActiveTab === DBOT_TABS.ACCUMULATORS
            ? 'accumulators'
    ...
    const is_purchase_received = display_contract_stage === (contract_stages.PURCHASE_RECEIVED as unknown);

    let progress_status = display_contract_stage - (is_purchase_sent || is_purchase_received ? 2 : 3);
    ...
    }, [is_profitdock_trade_running, is_stop_button_visible, is_stop_button_disabled]);
        ...
        <div className={classNames('animation__wrapper', className)}>
            ...
            <div
                className={classNames('animation__container', className, {
                    'animation--running': display_contract_stage > 0,
            ...
            >
                ...
                <span className='animation__text'>
                    {is_profitdock_trade_tab && profitdockTradeStatus?.label ? (
                        profitdockTradeStatus.label
                    ) : (
                        <ContractStageText contract_stage={display_contract_stage} />
                    )}
                </span>
                <div className='animation__progress'>
                    <div className='animation__progress-line'>
                        <div className={`animation__progress-bar animation__progress-${display_contract_stage}`} />
                    </div>
