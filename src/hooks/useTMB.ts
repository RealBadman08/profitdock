...
const useTMB = (): UseTMBReturn => {
    ...
    const isCallbackPage = useMemo(() => isOidcCallbackPath() || isRootOidcCallback(), []);
    ...
    const is_custom_legacy_oauth_domain = useMemo(() => isCustomLegacyOAuthDomain(), []);
    const isOAuth2Enabled = useMemo(
        () => !is_custom_legacy_oauth_domain && (is_production || is_staging),
        [is_custom_legacy_oauth_domain, is_production, is_staging]
    );
    ...
    const isTmbEnabled = useCallback(async () => {
        if (is_custom_legacy_oauth_domain) {
        ...
        return tmbStatusPromiseRef.current;
    }, [is_custom_legacy_oauth_domain, is_staging]);
                ...
                if (!activeSessions?.active && fromLoginButton) {
                    ...
                    try {
                        if (is_custom_legacy_oauth_domain) {
                            ...
                            window.location.replace(getCustomLegacyOAuthAuthorizeUrl());
                            return;
                        }
