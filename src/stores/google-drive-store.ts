...
import { importExternal } from '@/external/bot-skeleton/utils/html-helper';
...
export default class GoogleDriveStore {
    ...
    constructor(root_store: RootStore) {
        ...
        const is_profitdock_domain =
            typeof window !== 'undefined' && window.location.hostname.includes('profitdock.site');
        const has_google_drive_config = Boolean(this.client_id && this.app_id && this.api_key);

        if (!is_profitdock_domain && has_google_drive_config) {
            setTimeout(() => {
    ...
    initialise = () => {
        if (!this.discovery_docs || typeof gapi === 'undefined') return;
    ...
    initialiseClient = () => {
        if (!this.client_id || !this.scope || typeof google === 'undefined') return;
    ...
    async signIn() {
        if (!this.client) {
            botNotification(localize('Google Drive is not configured for this deployment.'), undefined, {
                closeButton: true,
            });
            return;
        }
