...
import { isCustomLegacyOAuthDomain, isProfitdockDomainHost } from '@/components/shared/utils/config/config';
...
import { ErrorTypes, MessageTypes, unrecoverable_errors } from '@/external/bot-skeleton/constants/messages';
import { observer } from '@/external/bot-skeleton/utils/observer';
import { getScratchUtils } from '@/utils/lazy-bot-skeleton';
...
const hasProfitdockTradingSession = () =>
    Boolean(api_base.is_authorized || api_base.has_authenticated_profitdock_socket);
...
export default class RunPanelStore {
    ...
    setShowBotStopMessage = (show_bot_stop_message: boolean) => {
        ...
        const handleNotificationClick = async () => {
            const { getSelectedTradeType } = await getScratchUtils();
    ...
    onRunButtonClick = async () => {
        ...
        const has_profitdock_recoverable_session =
            (isProfitdockDomainHost() || isCustomLegacyOAuthDomain()) && hasUsableProfitdockStoredSession();

        if (!client.is_logged_in) {
            if (!has_profitdock_recoverable_session) {
            ...
            const stored_loginid =
                localStorage.getItem('active_loginid') || Object.keys(JSON.parse(localStorage.getItem('accountsList') || '{}'))[0] || '';
            ...
            if (stored_accounts[stored_loginid]) {
                client.setAccountList(Object.values(stored_accounts));
            }

            client.setLoginId(stored_loginid);
            client.setCurrency(stored_accounts[stored_loginid]?.currency || client.currency);

            try {
                await api_base.init(true);
            } catch (error) {
                console.error('[ProfitDock Auth] Failed to refresh stored session before running bot:', error);
            }

            client.setIsLoggedIn(hasProfitdockTradingSession());
        }

        if (has_profitdock_recoverable_session && !hasProfitdockTradingSession()) {
            try {
                await api_base.init(true);
            } catch (error) {
                console.error('[ProfitDock Auth] Failed to refresh the trading session before starting the bot:', error);
            }
        }

        if ((isProfitdockDomainHost() || isCustomLegacyOAuthDomain()) && !hasProfitdockTradingSession()) {
            if (!hasUsableProfitdockStoredSession()) {
            ...
            this.showErrorMessage(
                localize(
                    'ProfitDock is still reconnecting to your Deriv trading session. Please wait a moment and try again.'
                )
            );
            return;
        }
    ...
    showLoginDialog = () => {
        ...
        this.onCancelButtonClick = () => {
            redirectToLogin(false, getInitialLanguage(), true, 0);
            this.is_dialog_open = false;
        };
        this.dialog_options = {
    ...
    showRealAccountDialog = () => {
        ...
        this.dialog_options = {
            title: localize("ProfitDock isn't ready for real accounts"),
    ...
    onBotStopEvent = () => {
        ...
        if (this.error_type === ErrorTypes.RECOVERABLE_ERRORS) {
            ...
            const { shouldRestartOnError = false, tradeEachTickEnabled, timeMachineEnabled = false } =
                this.dbot?.interpreter?.bot?.tradeEngine?.options ?? {};
            const is_bot_recoverable = shouldRestartOnError || tradeEachTickEnabled || timeMachineEnabled;
