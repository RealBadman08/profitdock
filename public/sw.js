...
const DISABLED_HOSTS = ['profitdock.site', 'www.profitdock.site'];
const SHOULD_DISABLE_SW = DISABLED_HOSTS.includes(self.location.hostname);
...
self.addEventListener('install', event => {
    if (SHOULD_DISABLE_SW) {
        console.log('[SW] Disabling service worker on ProfitDock host during install');
...
self.addEventListener('activate', event => {
    if (SHOULD_DISABLE_SW) {
        console.log('[SW] Disabling service worker on ProfitDock host during activation');
        event.waitUntil(
            (async () => {
