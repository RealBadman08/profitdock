    ...
    try {
        ...
        if (!loginid) {
            sendJson(res, 400, {
                ...
                message: 'ProfitDock could not determine which Deriv account to connect.',
            });
            return;
        }

        const payload = await fetchJson(
            `${DERIV_OPTIONS_ACCOUNTS_URL}/${encodeURIComponent(loginid)}/otp`,
            {
                headers: {
            ...
            'ProfitDock could not start an authenticated Deriv trading session.'
        );
        ...
        if (!websocketUrl) {
            sendJson(res, 502, {
                ...
                message: 'Deriv did not return an authenticated trading WebSocket URL.',
            });
            return;
        }
        ...
        console.error('[ProfitDock Options] Authenticated WebSocket URL request failed:', error);
        sendJson(res, error?.status || 500, {
            error: 'authenticated_websocket_failed',
            message:
